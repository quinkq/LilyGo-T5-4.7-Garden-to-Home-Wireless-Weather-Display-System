...
//Task handles
TaskHandle_t idleTaskHandle = NULL;
...
// Button ISR
static volatile TickType_t lastButtonTick = 0;
const TickType_t debounceTicks = pdMS_TO_TICKS(200);

void IRAM_ATTR ButtonISR() {
    TickType_t now = xTaskGetTickCountFromISR();
    if ((now - lastButtonTick) >= debounceTicks) {
...
// Variables to track last ESP-NOW reception time
...
{
    ESP_LOGI("ESP-NOW", "ESP-NOW - packet received from %02X:%02X:%02X:%02X:%02X:%02X, length: %d", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], len); // Log MAC too

    // Filter out false interrupt triggers or invalid packets
    ...
    // Stricter data check
    if (len != sizeof(SensorESPNOWData)) {
        ESP_LOGE("ESP-NOW", "ERROR: Received packet with incorrect length! Expected %d, Got %d. Discarding.", sizeof(SensorESPNOWData), len);
        // Optional: Log hex data for debugging short packets
        // if (len < 50) { // Avoid logging huge amounts of data if len is wrong
        //     char hex_buf[len * 3 + 1]; // Need space for hex chars, spaces, and null terminator
        //     for(int i=0; i<len; ++i) sprintf(hex_buf + i*3, "%02X ", transmissionData[i]);
        //     ESP_LOGE("ESP-NOW", "Data (hex): %s", hex_buf);
        // }
    ...
    memcpy(&espnowTEMPData, transmissionData, sizeof(SensorESPNOWData)); // Now safer due to length check
...
void rotateLogFile(const char *filename) {
    ...
    */
    // Improved Count lines
    int lineCount = 0;
    char buf[64];
    size_t bytes;
    while ((bytes = file.read((uint8_t*)buf, sizeof(buf))) > 0) {
...
void renderDisplay(volatile int &screenState) 
{
    ...
    // Checking screenState value correctness and by using "screens" calling appropriate Render_Screen function
    ...
    while (1) {
        ...
        if (xQueueReceive(csvLogQueue, &logEntry, portMAX_DELAY) == pdTRUE) {
            ...
            if(logEntry.timestamp < 1700000000){
                Serial.printf("Received data to log with incorrect timestamp, skipping loging.");
                continue;
            }
            ...
            // Round up timestamps to 10s
            logEntry.timestamp = (logEntry.timestamp + 5) / 10 * 10;
...
void IdleTask(void *pvParameters) {
    ESP_LOGI("IdleTask", "Started."); // <-- ADD LOG
    UpdateLocalTime();
    ...
    else 
    {
        ...
        if(xSemaphoreTake(ButtonWakeSem, SECONDS_TO_TICKS(sleepDuration)) == pdTRUE)
        {
        ...
        /* 
        // Previously used eventGroups to check for both ESPNOW or Button ISR semaphores, now just waiting for ButtonWakeSem above /\
        ...
        */
        ...
        //######################################################
        //Powersaving functions to be removed/deinitialized here//
        //######################################################
    ...
    ESP_LOGI("IdleTask", "Signaling completion and deleting task.");
...
void WeatherUpdateTask(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        xReturned = xTaskCreate(IdleTask, "IdleTask", 8192, NULL, 3, &idleTaskHandle);
        if (xReturned != pdPASS) 
        {
        ...
        // Waiting for idle to end before starting next cycle (included recovery mechanism in case something goes wrong..)
        if (xSemaphoreTake(idleEndedSem, MINUTES_TO_TICKS(16)) != pdTRUE) {
            ESP_LOGE("wUpdate", "Idle task stuck. Forcing system recovery.");

            if (idleTaskHandle != NULL) {
                vTaskDelete(idleTaskHandle);
                idleTaskHandle = NULL;
            }

            xReturned = xTaskCreate(IdleTask, "IdleTask", 8192, NULL, 5, &idleTaskHandle);
            if (xReturned == pdPASS) {
                ESP_LOGI("wUpdate", "IdleTask successfully restarted.");
            } else {
                ESP_LOGE("wUpdate", "Failed to recreate IdleTask. Restarting ESP...");
                esp_restart();
            }
        }
